...
export default function EditTab() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const saveResponse = await fetch(`/api/tabs/${id}`, {
        method: "PUT",
      ...
      if (saveResponse.ok) {
        console.log("Таб успішно додана в MongoDB:", saveResult);
      } else {
        console.error("Помилка при збереженні таба", saveResult);
      }
    } catch (error) {
  ...
  return (
    <div className={styles.container}>
      ...
      <h2 className={styles.title}>Редагування таба</h2>
      <form onSubmit={handleSubmit} className={styles.form}>
        ...
        >
          {isSending ? "Завантаження..." : "Оновити таб"}
        </button>
      </form>
    </div>
  );
